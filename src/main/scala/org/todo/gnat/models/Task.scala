...
case class Task(taskName: String, taskBody: Option[String] = None, taskOwner: Int, taskId: Option[Int] = None)
...
final class TaskTable(tag: Tag)
  ...
  def * =
    (taskName, taskBody.?, taskOwner, taskId.?) <> (Task.apply _ tupled, Task.unapply)
}
